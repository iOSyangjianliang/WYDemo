...
-(void)layoutSubviews
{
    ...
    [self reloadData:NO Item:[self getCurryPage]]; //Frame改变时同步刷新colloection和pagecontrol
}
...
-(void)reloadData:(BOOL)scrollToFirst Item:(NSInteger)lastItem
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        NSInteger item = self.infiniteDrag&&![self dataIsUnavailable]?1:0;
        if (self.arrayData.count>item && scrollToFirst) {
...
-(void)setInfiniteDrag:(BOOL)infiniteDrag
{
    ...
    NSInteger lastItem = [self getCurryPage];
    ...
    [self reloadData:NO Item:lastItem];
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.flowLayout.scrollDirection == UICollectionViewScrollDirectionVertical) {
        return CGSizeMake(self.jl_width, self.jl_height/(CGFloat)self.cellsOfLine);
    }else{
        CGFloat f = self.jl_width/(CGFloat)self.cellsOfLine;
        return CGSizeMake(self.jl_width/(CGFloat)self.cellsOfLine, self.jl_height);
    }
}
...
{
    if (self.pageControlNeed) {
        NSInteger page = [self getCurryPage];
        if (self.infiniteDrag) {
            if ([self dataIsUnavailable]) {
    ...
    if (self.collectionView.tracking && ![self dataIsUnavailable] &&self.infiniteDrag) { //解决两个手玩的情况
        CGFloat pageFloat;
        if (self.flowLayout.scrollDirection == UICollectionViewScrollDirectionVertical) {
            pageFloat = _collectionView.contentOffset.y / self.jl_height*(CGFloat)self.cellsOfLine;
        }else{
            pageFloat = _collectionView.contentOffset.x / self.jl_width*(CGFloat)self.cellsOfLine;
        }
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    if (self.pagingEnabled && self.cellsOfLine>1) {
        if (self.flowLayout.scrollDirection == UICollectionViewScrollDirectionVertical) {
            ...
            CGFloat page = roundf(targetContentOffsetCopy.y/self.jl_height*(CGFloat)self.cellsOfLine);
            CGFloat Y = self.jl_height/(CGFloat)self.cellsOfLine*page;
            *targetContentOffset = CGPointMake(targetContentOffsetCopy.x, Y);
            ...
            CGFloat X = self.jl_width/(CGFloat)self.cellsOfLine*page;
...
-(NSInteger)getCurryPage
{
    if (self.flowLayout.scrollDirection == UICollectionViewScrollDirectionVertical) {
        NSInteger page = roundf(_collectionView.contentOffset.y / self.jl_height*(CGFloat)self.cellsOfLine);
        return page;
    }else{
        NSInteger page = roundf(_collectionView.contentOffset.x / self.jl_width*(CGFloat)self.cellsOfLine);
        return page;
    }
}
#pragma mark - 自动翻页
