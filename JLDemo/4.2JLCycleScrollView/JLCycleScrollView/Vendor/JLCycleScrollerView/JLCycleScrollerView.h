...
@property(nonatomic) NSInteger cellsOfLine;    // 每行/列cell个数
@property(nonatomic) BOOL pagingEnabled;      //当每行/列cell个数大于一个时，是否按单个cell分页
//@property(nonatomic) NSInteger cellsOfScroll;  // 每次滚动cell个数，待开发
/**是否需要无限拖动，默认YES;*/
@property(nonatomic) BOOL infiniteDrag;

#pragma mark ------pageControl设置距离父视图边距-修改X.Y
/**同一方向(水平/垂直)属性设置互斥，即同一方向上多个设置只有最后一次设置生效
 eg:
...
#pragma mark -----定时器设置
/**是否需要定时器,默认YES,当视图移除时,定时器会自动进行销毁*/
