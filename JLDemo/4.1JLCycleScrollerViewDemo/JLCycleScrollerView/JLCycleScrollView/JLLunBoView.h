...
-(void)setTwoData:(nullable NSArray*)data WithCell:(UICollectionViewCell<JLLunboProtocol>*)cell isXibBuild:(BOOL)isxib;
...
//**-------------自定义扩展设置----------/待不断扩展以支持不同业务情景需求
@property(nonatomic,assign)UICollectionViewScrollDirection scrollDirection; //默认水平滚动
@property(nonatomic,assign)BOOL pageControlHidden; //默认NO
@property(nonatomic,assign)JLPageControlStyle pageControlStyle;
//在UIpageControl默认位置基础上调整X，Y(默认居中显示，距离底部10间距,高度10)
