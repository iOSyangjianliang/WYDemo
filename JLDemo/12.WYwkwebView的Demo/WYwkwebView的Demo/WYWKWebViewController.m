...
@interface WYWKWebViewController ()<WKNavigationDelegate,XLPhotoBrowserDatasource,XLPhotoBrowserDelegate, ZXEmptyViewControllerDelegate>
...
-(void) buildUI{
    ...
    self.parentViewController.navigationItem.leftBarButtonItems =items;
    //分享
    ...
    [self.parentViewController.navigationItem setRightBarButtonItems:itemsR animated:NO];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    else if ([keyPath isEqualToString:@"title"])
    {
        if (object == self.wkWebView)
        {
            self.parentViewController.navigationItem.title = self.wkWebView.title;
        }
        else
        {
            [super observeValueForKeyPath:keyPath ofObject:object change:change context:context];
...
-(void)setNavTitle:(NSDictionary *)dic{
    [self.parentViewController setTitle:[dic objectForKey:@"title"]];
...
-(void)setRight:(NSDictionary *)dic{
    ...
    if (btnArray.count == 1)
    {
        ...
        [self.parentViewController.navigationItem setRightBarButtonItems:items animated:YES];
    }
...
-(void)route:(NSDictionary *)dic{
    ...
    [[WYUtility dataUtil]routerWithName:[dic objectForKey:@"url"] withSoureController:self.parentViewController];
}
...
-(void)shareAction{
    ...
    [WYUTILITY shareSDKWithImage:imageStr Title:self.parentViewController.title Content:@"用了义采宝，生意就是好!" withUrl:self.webUrl];
}
...
- (UIBarButtonItem*)backButtonItem{
    if (!_backButtonItem) {
        ...
        [backButton setTitleColor:self.parentViewController.navigationController.navigationBar.tintColor forState:UIControlStateNormal];
        [backButton setTitleColor:[self.parentViewController.navigationController.navigationBar.tintColor colorWithAlphaComponent:0.5] forState:UIControlStateHighlighted];
...
- (void)updateNavigationItems
{
    if ([self.wkWebView canGoBack])
    {
        ...
        [self.parentViewController.navigationItem setLeftBarButtonItems:items animated:NO];
    }
    else
    {
        if (self.parentViewController.navigationItem.leftBarButtonItems.count>3)
        {
