...
-(void)addToTabbarController:(UITabBarController*)tabC WithTreeItemsArrayImages:(NSArray*)arrayimages;
{
    ...
    [self buildTabBarCopyBtnWithTabBarC];
...
-(void)TreeShow
{
    [self.tabC.view addSubview:self];
    ...
    //1.
    [self expendTabBarCopyRotation:YES WithTime:0.3];
    //2.
    [self expend:YES WithTime:0.3];
}
-(void)TreeRemoveWithAnimated:(BOOL)animated
{
    if (animated) {
        [self expendTabBarCopyRotation:NO WithTime:0.3];
        [self expend:NO WithTime:0.3];//动画end代理移除
        [UIView animateWithDuration:0.3 animations:^{
        ...
        [self expendTabBarCopyRotation:NO WithTime:0.1];
        [self expend:NO WithTime:0.1];
        
    }
    
}
#pragma mark -------动画---------
-(void)expendTabBarCopyRotation:(BOOL)isExtend WithTime:(CGFloat)time
{
    TreeItem*item = self.arrayMTreeItems.lastObject;
    if (isExtend) {
        if (item.center.x == item.endPoint.x && item.center.y == item.endPoint.y) {
            return;
        }
    }else{
        if (item.center.x == item.DefaultPoint.x && item.center.y == item.DefaultPoint.y) {
            return;
        }
    }
...
-(void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    ...
    if ([anim isEqual:[it.layer animationForKey:@"groupAnimation_End"]]) {
        ...
        //            an1.delegate = nil;//动画组设置成全局变量才会循环引用
        //            an2.delegate = nil;
        //        [self removeTreeViewAllAnimations];
...
-(void)removeTreeViewAllAnimations
{
    TreeItem* it = self.arrayMTreeItems.lastObject;
...
-(void)dealloc
{
    NSLog(@"treeView能够dealloc");
}

- (void)expend:(BOOL)isExpend WithTime:(CGFloat)time
{
    
    [self.arrayMTreeItems enumerateObjectsUsingBlock:^(TreeItem *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [self addAnimationWithTreeItems:obj toShow:isExpend WithTime:time];
        //        NSLog(@"%ld-----",idx);
    }];
}
-(void)addAnimationWithTreeItems:(TreeItem *)item toShow:(BOOL)show WithTime:(CGFloat)time
{
    if (show) {
        if (item.center.x == item.endPoint.x && item.center.y == item.endPoint.y) {
            return;
...
{
    [self TreeRemoveWithAnimated:NO];
...
#pragma mark - 覆盖相同按钮
-(void)buildTabBarCopyBtnWithTabBarC
{
    CGFloat www = 10000;
    NSMutableArray *tabbarbuttonArray = [NSMutableArray array];
    for (UIControl *tabBarButton in self.tabC.tabBar.subviews){
    ...
    if (tabbarbuttonArray.lastObject) {
        ...
        self.TabBarCopyBtn = [[UIButton alloc] initWithFrame:frame];
        ...
        self.TabBarCopyBtn.backgroundColor = [WYUISTYLE colorWithHexString:@"FAFAFA"];
        ...
        for (UIView *imageView in tabBarButton.subviews) {
            if ([imageView isKindOfClass:NSClassFromString(@"UITabBarSwappableImageView")]) {
                ...
                self.imageViewCopy.image = [[UIImage imageNamed:@"btn-+"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
                NSLog(@"%@",NSStringFromCGRect(self.imageViewCopy.frame));
            }
        }
...
{
    [self TreeRemoveWithAnimated:YES];
}
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.backgroundView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, SCR_W, SCR_H-50)];
        self.backgroundView.backgroundColor = [UIColor blackColor];
        self.backgroundView.alpha = 0;
        [self addSubview: self.backgroundView];
        self.TabBarbackgroundView = [[UIImageView alloc] initWithFrame:CGRectMake(0,SCR_H-50, SCR_W,50)];
